...
# ============================================================
# TAB 5 - MANUAL SINGLE SAMPLE PREDICTION
# ============================================================
with tab5:
    st.header("Stage 5: Predict One New Soil Sample")

    st.write(
        "Enter one complete set of input values manually. "
        "The trained ANN will predict the selected output property."
    )

    if st.session_state["trained_model"] is None:
        st.warning(
            "Please train the ANN model first."
        )
    else:
        trained = st.session_state["trained_model"]
        features = trained["features"]
        target = trained["target"]
        reference_data = trained["X_train"]

        st.success(
            f"Current ANN model predicts: {target}"
        )

        st.info(
            "For each selected input feature, enter one value. "
            "The app will then predict one output value."
        )
        ...
        for feature in features:
            ...
            if len(values) > 0:
                st.caption(
                    f"Training range for {feature}: "
                    f"{float(values.min()):.4f} to "
                    f"{float(values.max()):.4f}"
                )

            # Strictly restrict user input to the observed
            # minimum and maximum values in the training dataset.
            if len(values) > 0:
                min_value = float(values.min())
                max_value = float(values.max())

                st.caption(
                    f"Allowed range for {feature}: "
                ...
                manual_values[feature] = st.number_input(
                    ...
                    help=(
                        f"Only values between {min_value:.4f} "
                        f"and {max_value:.4f} are allowed."
                    ),
                )
            else:
                st.error(
                    f"No valid numerical range is available for {feature}."
                )

        if st.button(
            f"🎯 Predict {target}",
            type="primary",
            key="manual_prediction_button",
        ):
            try:
                ...
                predicted_value = float(
                    trained["model"].predict(new_sample)[0]
                )

                st.session_state["manual_prediction"] = {
                    "target": target,
                    "prediction": predicted_value,
                    "inputs": manual_values.copy(),
                }
        ...
        # Keep the result visible after Streamlit reruns.
        if "manual_prediction" in st.session_state:
            ...
            if result["target"] == target:
                ...
                st.success(
                    "✓ Input values are strictly limited to the "
                    "minimum and maximum ranges of the training dataset."
                )
